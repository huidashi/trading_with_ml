...
def row_port_value(row, holdings, symbols_list, cash_balance, impact, commission):
    ...
    if pd.notna(row['Symbol']) and pd.notna(row['Order']):
        ...
        if row['Order'] =='BUY':
            ...
            total_cost = adjusted_price_impact * row['Shares'] + commission
            cash_balance = cash_balance - total_cost
            holdings[row['Symbol']] = holdings[row['Symbol']] + row['Shares']
        #sell stock
        elif row['Order'] == 'SELL':
            adjusted_price_impact = stock_price * (1-impact)
            total_revenue = adjusted_price_impact * row['Shares'] - commission
...
def compute_portvals(  		  	   		 	   		  		  		    	 		 		   		 		  
    ...
    #read
    orders_df = pd.read_csv(orders_file, index_col=0, parse_dates=True, na_values=['nan']).sort_index()
    #dates
